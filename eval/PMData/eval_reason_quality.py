...
def evaluate_with_Api(reasoning_text, question, dimension, client, model, temperature):
    """使用API评估单个维度（同步版本）"""
    try:
        if dimension == 'readability':
            prompt = get_readability_prompt(reasoning_text)
        elif dimension == 'logical_consistency':
            prompt = get_logical_consistency_prompt(reasoning_text, question)
...
async def evaluate_with_Api_async(reasoning_text, question, dimension, client, model, temperature, semaphore):
    """使用API评估单个维度（异步版本）
...
async def evaluate_single_item_async(item, client, model, temperature, semaphore):
    ...
    # 为每个维度创建任务
    tasks = [
        evaluate_with_Api_async(reasoning_text, question, dim, client, model, temperature, semaphore)
...
def run_evaluation(inference_file, output_file, api_key, base_url, model, temperature=0.0):
    ...
    for item in evaluation_results:
        key = f"{item['user_id']}_{item['subtask']}_{item['time_frame']}"
    ...
    if not remaining_results:
        ...
        for item in tqdm(remaining_results, desc="Evaluating"):
            ...
            for dimension in dimensions:
                eval_result = evaluate_with_Api(reasoning_text, question, dimension, client, model, temperature)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Run evaluation on PMData inference results",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument(
        "--inference-files", "-i",
        nargs="+",
        default=['/path/to/inference_results.json'],
        help="List of inference result files"
    )
    parser.add_argument(
        "--output-files", "-o",
        nargs="+",
        default=['/path/to/eval_output.json'],
